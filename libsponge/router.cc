...
void Router::route_one_datagram(InternetDatagram &dgram) {
    ...
    for (uint32_t i = 0; i < _routes.size(); i++) {
        ...
        auto route_prefix_masked = static_cast<uint64_t>(route_prefix) >> mask_length;
        if (dgram_dst_masked != route_prefix_masked) {
            continue;
        }
        if (j == nullopt || prefix_length > get<1>(_routes[j.value()])) {
            j = i;
        }
    }
