...
class StreamReassembler {
  ...
  public:
    ...
    //! `data` \param eof the last byte of `data` will be the last byte in the
    //! entire stream
    void push_substring(const std::string &data, const uint64_t index, const bool eof);

    //! \name Access the reassembled byte stream
    //!@{
    inline const ByteStream &stream_out() const { return _output; }
    inline ByteStream &stream_out() { return _output; }
    //!@}
